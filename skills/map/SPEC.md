...
Machine-level project identity/registration metadata is separately defined in section 31. It is not semantic graph or recovery-session data.
...
- project move/path refresh;
- duplicate-project identity detection and user-directed separation/removal;
- corrupted/mismatched project identity rejection and explicit recovery;
...
- synchronization of duplicated Map projects;
...
## 31. Map project identity and registry

Map owns a machine-level project registry at:

```text
~/.jl-skills/map/registry.json
```

This registry is separate from the generic `jl-skill` installer registry. The installer registry is authoritative for installer-managed skill integrations; the Map registry is authoritative for Map projects created by the Map runtime.

The Map registry is operational identity/location metadata, not semantic graph state. The `.map/` graph remains authoritative for intents, questions, decisions, facts, ideas, relationships, history, and recovery-session data.

### 31.1 Project identity

Every initialized Map has one stable opaque **project ID** stored as Map-owned metadata in the `.map` container.

A registry entry contains at least:

```text
project ID
canonical/normalized project path
created timestamp
last-seen timestamp where useful
```

The exact serialization and project-ID representation are implementation details unless another contract requires them. Project IDs must be generated by Map, not accepted as arbitrary normal `map init` input.

### 31.2 Registration is part of `map init`

Successful `map init` must create both:

1. the new `.map` state with its project ID; and
2. the corresponding registry entry.

These are one logical initialization operation. A new `map init` must not report success while leaving an intentionally unregistered Map behind.

If registration cannot be made durable during new initialization, fail and roll back the newly-created Map state where practical rather than knowingly creating an orphaned project.

The `jl-skill` installer must never create project-registry entries because installing the skill does not initialize a Map project.

### 31.3 Registry authority boundary

For V1 lifecycle behavior, the Map registry is authoritative for known Map projects.

Map is not required to crawl user filesystems looking for `.map` containers that have never been registered through the supported lifecycle. A user manually reconstructing, restoring, copying, or altering Map-owned identity state outside the tooling is responsible for entering an explicit recovery path when the runtime detects an inconsistency.

Do not silently adopt an unknown Map merely because a `.map` directory exists.

### 31.4 Normal open and moved projects

When normal Map operation resolves an existing `.map`, compare its stored project ID/path against the registry before performing semantic work.

If the project ID is registered to the current path, proceed normally and refresh `lastSeen` where maintained.

If the project ID is registered to another path and that registered original Map no longer exists there, treat this as an ordinary project move/rename:

```text
registered: C:\Old\Project
opened:     D:\New\Project
same project ID
old registered Map no longer present
```

Update the registry path to the newly resolved project location and proceed. No semantic graph rewrite is required.

Merely retaining a stale registry entry after a project was manually deleted is acceptable until the project is opened elsewhere or an explicit cleanup/removal operation processes the registry.

### 31.5 Duplicate copied projects

If the runtime opens a Map whose project ID is registered to another location **and the original registered Map still exists there with the same project ID**, the project has been copied rather than moved.

Map must not:

- allow two active registered locations to continue sharing one project identity;
- attempt to synchronize the copies;
- guess which copy is authoritative;
- silently mint a new identity without user direction.

Normal semantic operation stops until the ambiguity is resolved.

In an interactive terminal, present a simple user choice equivalent to:
...
`Keep both and make this a separate Map` preserves the semantic contents of the current copy, generates a fresh project ID for the current copy, and registers it independently. It does not rewrite ordinary graph node IDs or semantic history merely because the project identity changed.

`Remove one of these Maps` must ask which copy the user intends to remove and require an explicit destructive confirmation before deleting `.map` project data. It must not delete neighboring project files.

If the caller is non-interactive, do not make the ownership choice automatically. Return a clear duplicate-project error containing enough location information for a user to resolve it explicitly.

### 31.6 Registry/path contradictions

When checking a possible move/duplicate, the registered path must be inspected narrowly enough to distinguish:

- no Map present there: ordinary move is allowed;
- a Map with the same project ID present there: duplicate-copy flow;
- a Map present there with a different/malformed identity: registry/project-identity inconsistency; stop and require recovery rather than guessing.

Directory existence alone is not proof that the original Map still exists.

### 31.7 Corrupted or manually altered identity

If a resolved `.map` has missing, malformed, or conflicting project identity metadata, or the registry says a different project identity belongs at that path, normal semantic operation must stop.

Do not automatically recover by path matching, timestamps, graph contents, or other heuristics.

When interactive recovery is appropriate, present a user-directed recovery choice equivalent to:

```text
This Map's project identity is damaged or does not match the registry.

> Register and recover this Map
  Cancel
```

Recovery must first validate the existing Map data as far as possible without relying on the damaged identity.

If structural/semantic validation succeeds, the recovery operation may restore/generate a valid project identity and write the corresponding authoritative registry entry with explicit user approval.

If validation fails materially, recovery stops with a fatal error. Do not promise or attempt speculative reconstruction of corrupted graph state.

### 31.8 Registry use during removal

Machine-level JL-Skills removal may use the Map registry to enumerate the Map projects JL-Skills knows about. It must not recursively search user drives for unregistered `.map` directories.

Deleting Map project data remains a separate explicit destructive choice from uninstalling skill integrations or JL-Skills program files. Missing registered paths are reported/skipped and may have stale registry records cleaned as part of an explicit removal operation.

### 31.9 Simplicity boundary

The V1 registry lifecycle intentionally uses simple primitives:

- generate/register on init;
- verify identity on open;
- refresh path for an unambiguous move;
- stop and ask on a duplicate copy;
- stop and offer explicit validation/re-registration on identity corruption;
- fail when safe recovery cannot be established.

Do not expand this into distributed synchronization, filesystem archaeology, automatic merge/reconciliation, or increasingly clever heuristics merely to rescue unsupported manual states.